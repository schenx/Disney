---
title: "Wait Time Machine Learning Models for Expedition Everest"
author: "Steven Chen"
date: "June 10, 2019"
output:
...
A simple linear regression model was created using factors such as DAYOFWEEK, DAYOFYEAR, TimeSec, etc. as used in the visualzations. 
...
Using a KNN and Random Forest ensemble approach resulted in the most accurate wait time prediction model. The RMSE of 10.3, while an improvement of 3.8 minutes over the Naive model, still seems relatively large.  In the future it would be good to include the categorical data (converting to binrary or other numerical values) as well as data from other theme parks to see if this has an impact on the RMSE.  What's not clear is how to acutally use the model to predict lower times.  Perhaps this can be determined by applying the y_hat to the validation data to see what factors are the greatest attribute. 
